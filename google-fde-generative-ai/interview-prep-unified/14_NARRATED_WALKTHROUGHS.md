...
**00:00 — Candidate (opener, from the README):** Thanks Priya. I am Kaustabh Ganguly, five and a half years in AI and ML, currently senior ML engineer at Gracenote where I own the agentic platform for video and music metadata enrichment. I shipped an enterprise agentic stack on LangGraph with Claude Haiku that cut p95 latency roughly in half and brought serving cost down about threefold, with DSPy-based prompt compilation and DeepEval in the CI loop so regressions fail the build, not production. Before Gracenote I spent three years at Johnson and Johnson MedTech on a clinical AI platform that hit 0.95 AUROC on surgical complication prediction and is now deployed in six hospitals. On the side, I have shipped two npm MCP servers, one is the OAuth-backed Data Sentry connector for enterprise tool integration, and I ran a small research harness called AutoResearch and a kiro-cli scaffolder that together drove an embedding fine-tuning run on EmbeddingGemma 300M and got me about twelve points of accuracy lift on a domain benchmark with Matryoshka truncation preserved. The thing I enjoy most is taking a fuzzy customer problem and turning it into a concrete architecture plus an eval harness plus a cost budget, all three at once, which is why this role is interesting to me.
...
This is the same security spine I have built before. At J&J MedTech the clinical platform was under HIPAA with per-hospital ACLs enforced at retrieval and audit logging of every model call, and my Data Sentry MCP server is literally the OAuth-backed enterprise connector pattern, reused. The shapes here are familiar; the RBI variant changes the perimeter vocabulary and the residency constraint.
